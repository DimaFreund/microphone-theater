...
class Customers_List extends WP_List_Table {
	...
	public static function get_customers( $per_page = 5, $page_number = 1 ) {
		...
		$sql .= " LEFT JOIN {$wpdb->prefix}posts ON {$wpdb->prefix}orders.product = {$wpdb->prefix}posts.ID";
	...
	function get_columns() {
		$columns = array(
			...
			'order_id' => __( 'id на сайте', 'sp' ),
			'post_title'    => __( 'Спектакль', 'sp' ),
			'price' => __( 'Цена', 'sp' ),
			'id'    => __( 'id в системе Платон', 'sp' ),
			'email' => __( 'E-mail', 'sp' ),
			'name' => __( 'Имя', 'sp' ),
			'date_create' => __( 'Дата заказа', 'sp' ),
			'ip_address' => __( 'IP адрес', 'sp' ),
			'payment_data' => __( 'Маска плетежной карты', 'sp' ),
		);
	...
	public function get_sortable_columns() {
		$sortable_columns = array(
			'date_create' => array( 'date_create', true ),
